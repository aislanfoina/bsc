...
int compareELMsort (ELM *a, ELM *b) {
	if(a->ngram < b->ngram)
		return -1;
...
int compareMenorELM (ELM a, ELM b) {
	return(a.ngram < b.ngram);
}
int compareMaiorELM (ELM a, ELM b) {
	return(a.ngram > b.ngram);
}
int compareMenorIgualELM (ELM a, ELM b) {
	return(a.ngram <= b.ngram);
/*	if(a->ngram < b->ngram) {
		return 1;
	} else if(a->ngram == b->ngram && a->index < b->index)
		return 1;
	else
		return 0;*/
}
...
/*int compareMaiorIgualELM (ELM a, ELM b) {
	return(a.ngram >= b.ngram);
	if(a->ngram > b->ngram) {
		return 1;
	} else if(a->ngram == b->ngram && a->index > b->index)
...
//                for (p = q - 1; p >= low && (b = p[0]) > a; p--)
                for (p = q - 1; p >= low && compareMaiorELM(b, a); p--) {
                        p[1] = b;
                        b = p[0];
                }
                p[1] = a;
...
{
//        if (a < b)
		if (compareMenorELM(a, b))
        {
//                if (b < c)
	        if (compareMenorELM(b, c))
                        return b;
                else
                {
//                        if (a < c)
						if (compareMenorELM(a, c))
                                return c;
                        else
                                return a;
                }
        ...
        {
//                if (b > c)
				if (compareMaiorELM(b, c))
                        return b;
                else
                {
//                        if (a > c)
						if (compareMaiorELM(a, c))
                                return c;
                        else
                                return a;
                }
...
//                while ((h = *curr_high) > pivot)
				h = *curr_high;
				while (compareMaiorELM(h, pivot)) {
                        curr_high--;
                        h = *curr_high;
				}
				l = *curr_low;
//                while ((l = *curr_low) < pivot) {
                while (compareMenorELM(l, pivot)) {
                        curr_low++;
                        l = *curr_low;
                }
...
{
		qsort(low, high-low+1, sizeof(ELM),compareELMsort);
...
//                if (val <= *mid)
				if (compareMenorIgualELM(val, *mid))
                        high = mid - 1;
                else
                        low = mid;
        }
