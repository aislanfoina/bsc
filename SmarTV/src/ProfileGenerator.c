 ...
 */
		...
		saveDataProfile(id, totalCount, totalMean2, totalStd2, "profiles_Data", movieDb, conn);
		saveIntProfile(id, (int *)genCounter, "profiles_RateCounter", movieDb, conn);
		...
		saveFloatProfile(id, genCntPer, "profiles_RateCntPer", movieDb, conn);
		...
		saveFloatProfile(id, genNormal, "profiles_RateNormal", movieDb, conn);
...
//	MYSQL_ROW row;
	...
	sprintf(query,"insert into %s_%s values (%d,"
			"%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,"
			"%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,"
			"%d,%d,%d,%d,%d,%d,%d,%d);",table,moviedb,userId,
			profile[0],profile[1],profile[2],profile[3],profile[4],
			profile[5],profile[6],profile[7],profile[8],profile[9],
			profile[10],profile[11],profile[12],profile[13],profile[14],
			profile[15],profile[16],profile[17],profile[18],profile[19],
